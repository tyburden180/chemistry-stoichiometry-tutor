...
function renderPath(problem) {
  ...
  problem.steps.forEach((step, index) => {
    ...
    const status = state.answers[answerKey(state.problemIndex, index)]?.isCorrect ? "Completed" : index === state.stepIndex ? "Current step" : "Waiting";
    card.innerHTML = `<strong>${index + 1}. ${step.skill}</strong><p>${status}</p><div class="formula">${index <= state.stepIndex ? step.work : ""}</div>`;
