...
function render() {
  ...
  elements.nextButton.disabled = !saved?.isCorrect;
...
function renderChoices(step, saved) {
  ...
  step.choices.forEach((choice, index) => {
    ...
    if (saved) {
      ...
      if (saved.isCorrect) button.disabled = true;
    }
...
function renderFeedback(step, saved) {
  ...
  if (saved.isCorrect) {
    ...
    elements.feedback.innerHTML = `<p><strong>Try that step again.</strong> ${saved.reason}</p>`;
  }
}
...
function renderPath(problem) {
  ...
  problem.steps.forEach((step, index) => {
    ...
    const isComplete = state.answers[answerKey(state.problemIndex, index)]?.isCorrect;
    const status = isComplete ? "Completed" : index === state.stepIndex ? "Current step" : "Waiting";
    card.innerHTML = `<strong>${index + 1}. ${step.skill}</strong><p>${status}</p><div class="formula">${isComplete ? step.work : ""}</div>`;
...
function renderResults() {
  ...
  state.test.forEach((problem, problemIndex) => {
    const completed = problem.steps.filter((_, stepIndex) => state.answers[answerKey(problemIndex, stepIndex)]?.isCorrect).length;
    const card = document.createElement("div");
...
function isTestComplete() {
  return state.test.every((problem, problemIndex) =>
    problem.steps.every((_, stepIndex) => state.answers[answerKey(problemIndex, stepIndex)]?.isCorrect)
  );
}
...
function chooseAnswer(index) {
  ...
  const alreadyCorrect = state.answers[key]?.isCorrect;
